    ...
    route("metrics") {
        route("{id}") {
            get("line") {
                ...
                call.respond(ClickHouseMetricRepository.getMetricInDateRange(id, from, to, serverSide))
            }
            get("pie") {
                val id = call.parameters["id"]?.toIntOrNull() ?: return@get call.respondMessage(
                    HttpStatusCode.BadRequest, "Incorrect project ID"
                ...
                call.respond(ClickHouseMetricRepository.getMetricCountById(id, serverSide))
            }
        }
        post {
            ...
            when (ClickHouseMetricRepository.add(metrics)) {
