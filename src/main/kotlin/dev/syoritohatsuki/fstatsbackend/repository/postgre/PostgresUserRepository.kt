...
object PostgresUserRepository : UserRepository {
    ...
    override suspend fun getById(id: Int): User? = dbQuery {
        UsersTable.selectAll().where { UsersTable.id eq id }.mapNotNull(User::fromResultRow).singleOrNull()
    }

    override suspend fun getByName(username: String): User? = dbQuery {
        UsersTable.selectAll().where { UsersTable.username eq username }.mapNotNull(User::fromResultRow).singleOrNull()
    }
